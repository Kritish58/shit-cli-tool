      ...
      exec(`rm -r ${stdout.toString().replace(/(\r\n|\n|\r)/gm, '') + module_name}`, (error, stdout, stderr) => {
         if (error) {
            ...
            if (error.message.includes('No such file or directory')) {
               console.log(
                  RED_TEXT_CODE,
                  module_name.split('\\')[module_name.split('\\').length - 1] + ' module does not exist'
               );
            }
            return;
         }
         ...
         console.log(GREEN_TEXT_CODE, module_name.split('\\')[module_name.split('\\').length - 1] + ' module removed');
      });
   });
};
