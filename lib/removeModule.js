      ...
      exec(`rm -r ${stdout.toString().replace(/(\r\n|\n|\r)/gm, '') + module_name}`, (error, stdout, stderr) => {
         if (error) {
            ...
            if (error.message.includes('No such file or directory')) {
               console.log(
                  chalk.bold.red(module_name.split('\\')[module_name.split('\\').length - 1] + ' module does not exist')
               );
            }
            return;
         }
         ...
         console.log(chalk.bold.green(module_name.split('\\')[module_name.split('\\').length - 1] + ' module removed'));
      });
   });
};
